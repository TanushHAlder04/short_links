// lib/analytics.js
// Awaitable click recorder, called from the redirect's after() callback.
...
async function triggerMilestoneWebhook(urlRecord, clickCount) {
  ...
  try {
    ...
    await sendWebhook(urlRecord.webhookUrl, payload, headers)
  } catch (err) {
 ...
 */
export async function recordClick({ shortCode, ip, userAgent, referrer }) {
  const isBot = isBotUserAgent(userAgent)
  try {
    const [uaData, geoData] = await Promise.all([parseUserAgent(userAgent), getGeoData(ip)])
    // Either both the event and human counter commit, or neither does.
    const updatedUrl = await prisma.$transaction(async (tx) => {
      await tx.click.create({ data: {
        shortCode, ipHash: hashIP(ip), ...geoData, ...uaData,
        referrer: referrer || null, isBot,
      } })
      if (isBot) return null
      return tx.url.update({
        where: { shortCode }, data: { clickCount: { increment: 1 } },
        select: { shortCode: true, clickCount: true, webhookUrl: true, webhookSecret: true },
      })
    })
    if (updatedUrl) await triggerMilestoneWebhook(updatedUrl, updatedUrl.clickCount)
  } catch (err) {
